...
export default function Directory() {
  ...
  const displayPrograms = useMemo(() => {
    ...
    if (sortBy === "closing_soon") {
      sorted.sort((a, b) => {
        ...
        return new Date(a.registrationCloseDate).getTime() - new Date(b.registrationCloseDate).getTime();
      });
    } else if (sortBy === "a_z") {
  ...
  const FilterPanel = () => (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Sport</label>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 lg:hidden flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 pb-12 animate-in slide-in-from-bottom max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-display font-bold text-xl text-slate-800">Refine Search</h3>
            ...
            <div className="mt-8 flex gap-3">
              <button onClick={clearAllFilters} className="flex-1 py-3 text-slate-600 font-bold bg-slate-100 rounded-xl">Clear All</button>
              <button onClick={() => setShowMobileFilters(false)} className="flex-1 py-3 bg-[#4A8C2A] text-white font-bold rounded-xl shadow-lg">Apply</button>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="p-6 overflow-y-auto space-y-6 flex-1">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex gap-4">
                    <div className="flex-1">
                      <p className="text-xs font-bold text-slate-400 uppercase mb-1">Registration</p>
                      <p className="font-semibold text-slate-800">
                         {selectedProgram.registrationOpenDate ? new Date(selectedProgram.registrationOpenDate).toLocaleDateString() : "TBD"} 
                         {" - "}
                         {selectedProgram.registrationCloseDate ? new Date(selectedProgram.registrationCloseDate).toLocaleDateString() : "TBD"}
                      </p>
                    </div>
                    <div className="flex-1 border-l border-slate-200 pl-4">
                      <p className="text-xs font-bold text-slate-400 uppercase mb-1">Season Starts</p>
                      <p className="font-semibold text-slate-800">
                        {selectedProgram.programStartDate ? new Date(selectedProgram.programStartDate).toLocaleDateString() : "TBD"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-slate-800">{selectedProgram.townArea}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-slate-400 uppercase">Age Groups</p>
                        <p className="font-medium text-slate-800">{selectedProgram.ageGroups}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedProgram.notes && (
                    <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-2 text-[#1B3A6B]">
                        ...
                        <span className="font-bold text-sm uppercase tracking-wider">Details & Notes</span>
                      </div>
        ...
        <div className="bg-slate-50 border-b border-slate-200 shadow-sm relative z-20">
          <div className="max-w-6xl mx-auto px-6 lg:px-8 py-4 flex justify-center">
            {topAds.map((ad: any) => (
              ...
              >
                <div className="absolute -top-2 -right-2 bg-slate-800 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity z-10">Sponsor</div>
                <img
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {displayPrograms.map((program, idx) => (
                  <React.Fragment key={program.id}>
                    ...
                    <ProgramCard program={program} onMoreInfo={() => setSelectedProgram(program)} />
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                <h3 className="font-display font-bold text-slate-800 mb-4 border-b border-slate-100 pb-3 flex items-center justify-between">
                  Refine Search
                  {activeFilters.length > 0 && (
                    <button onClick={clearAllFilters} className="text-xs font-semibold text-slate-400 hover:text-slate-700 transition-colors">Clear</button>
                  )}
                </h3>
                <FilterPanel />
              </div>
              ...
              {sidebarAds && sidebarAds.length > 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-dashed border-slate-300 shadow-sm p-6 text-center">
                  ...
                  <p className="text-xs text-slate-500 mb-4">Support local youth sports and reach thousands of parents.</p>
                  <a href="https://link.convertmorebusiness.com/widget/form/1CCxDZ3WadfM9IbjIZCk" target="_blank" rel="noopener noreferrer" className="inline-block bg-[#1B3A6B] text-white text-xs font-bold px-4 py-2 rounded-lg hover:bg-blue-900 transition-colors">Learn More</a>
                </div>
              )}
            </div>
          </div>
      ...
      <section id="alerts" className="bg-slate-900 py-20 px-6 lg:px-8 relative overflow-hidden border-t border-slate-800">
...
function ProgramCard({ program, onMoreInfo }: { program: any; onMoreInfo: () => void }) {
        ...
        <div className="space-y-3 mb-5 py-4 border-y border-slate-100 flex-1">
          ...
          <div className="flex gap-2.5 items-center">
            ...
            <p className="text-sm font-medium text-slate-800">
              <span className="text-slate-500 font-normal mr-1">Reg Window:</span>
              {formatDate(program.registrationOpenDate)} - {formatDate(program.registrationCloseDate)}
            </p>
          </div>
          ...
          <div className="flex gap-2.5 items-center">
            ...
            <p className="text-sm font-medium text-slate-800 line-clamp-1">
              <span className="text-slate-500 font-normal mr-1">Season Starts:</span>
