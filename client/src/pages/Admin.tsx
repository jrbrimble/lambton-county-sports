...
const emptyProgram: ProgramFormData = {
  sportName: "", organization: "", ageGroups: "",
  registrationOpenDate: "", registrationCloseDate: "", programStartDate: "",
  registrationUrl: "", notes: "", isActive: true,
};
...
function ProgramDialog({
  ...
  const createMut = trpc.programs.create.useMutation({
    onSuccess: () => { utils.programs.listAll.invalidate(); toast.success("Program created"); onClose(); },
    onError: (e) => toast.error(e.message),
  });
  const updateMut = trpc.programs.update.useMutation({
    onSuccess: () => { utils.programs.listAll.invalidate(); toast.success("Program updated"); onClose(); },
    onError: (e) => toast.error(e.message),
  });
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input value={form.sportName} onChange={(e) => setForm({ ...form, sportName: e.target.value })} required />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <Label>Age Groups * <span className="text-muted-foreground font-normal">(e.g. U6, U8, U10)</span></Label>
            <Input value={form.ageGroups} onChange={(e) => setForm({ ...form, ageGroups: e.target.value })} required />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input type="url" value={form.registrationUrl} onChange={(e) => setForm({ ...form, registrationUrl: e.target.value })} required placeholder="https://..." />
          </div>
          <div className="space-y-1.5">
            <Label>Notes</Label>
...
function ProgramsTab() {
  ...
  const deleteMut = trpc.programs.delete.useMutation({
    onSuccess: () => { utils.programs.listAll.invalidate(); toast.success("Program deleted"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  function openCreate() { setEditProgram(undefined); setDialogOpen(true); }
  function openEdit(p: SportsProgram) { setEditProgram(p); setDialogOpen(true); }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">{programs?.length ?? 0} total listings</p>
        </div>
        ...
        <div className="rounded-xl border border-border overflow-hidden bg-card">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Sport / Organization</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground hidden md:table-cell">Age Groups</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground hidden lg:table-cell">Reg. Open</th>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground hidden lg:table-cell">Reg. Close</th>
            ...
            <tbody className="divide-y divide-border">
              {programs?.map((p) => (
                <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">{p.ageGroups}</td>
                  <td className="px-4 py-3 text-muted-foreground hidden lg:table-cell">{formatDate(p.registrationOpenDate)}</td>
                  <td className="px-4 py-3 text-muted-foreground hidden lg:table-cell">{formatDate(p.registrationCloseDate)}</td>
                  <td className="px-4 py-3">
                    <span className={p.isActive ? "badge-open" : "badge-closed"}>
      ...
      <ProgramDialog open={dialogOpen} onClose={() => setDialogOpen(false)} program={editProgram} />

      <AlertDialog open={deleteId !== null} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this program?</AlertDialogTitle>
            <AlertDialogDescription>This action cannot be undone. The listing will be permanently removed from the directory.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function ChangesTab() {
  ...
  const approveMut = trpc.changes.approve.useMutation({
    onSuccess: () => { utils.changes.listPending.invalidate(); utils.changes.pendingCount.invalidate(); toast.success("Change approved and applied"); },
    onError: (e) => toast.error(e.message),
  });
  const dismissMut = trpc.changes.dismiss.useMutation({
    onSuccess: () => { utils.changes.listPending.invalidate(); utils.changes.pendingCount.invalidate(); toast.success("Change dismissed"); },
    onError: (e) => toast.error(e.message),
  });
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-0.5">
          Auto-detected date changes from the monthly URL check. Review and approve or dismiss each one.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-card rounded-xl border border-border">
          ...
          <p className="text-muted-foreground mt-1">No pending changes to review.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {changes?.map((change) => (
            <div key={change.id} className="bg-card rounded-xl border border-border p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    Field: <span className="font-medium text-foreground">{FIELD_LABELS[change.fieldName] ?? change.fieldName}</span>
                  </p>
                  <div className="flex items-center gap-3 text-sm">
                    <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-1.5">
                      <p className="text-xs text-red-500 font-medium mb-0.5">Current</p>
                      <p className="text-red-700">{change.oldValue || "Not set"}</p>
                    </div>
                    <ChevronRight className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1.5">
                      <p className="text-xs text-emerald-600 font-medium mb-0.5">Detected</p>
                      <p className="text-emerald-700">{change.newValue || "Not set"}</p>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground mt-2">
                    Detected {new Date(change.detectedAt).toLocaleDateString("en-CA", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                  </p>
                </div>
...
const emptyAd: AdFormData = {
  title: "", destinationUrl: "", position: "sidebar_card", sortOrder: 0, isActive: true,
};
...
function AdDialog({ open, onClose, ad }: { open: boolean; onClose: () => void; ad?: AdSlot }) {
  const utils = trpc.useUtils();
  const fileRef = useRef<HTMLInputElement>(null);
  const [form, setForm] = useState<AdFormData>(
    ad
      ? { title: ad.title, destinationUrl: ad.destinationUrl, position: ad.position, sortOrder: ad.sortOrder, isActive: ad.isActive }
      : emptyAd
  );
  ...
  const createMut = trpc.ads.create.useMutation({
    onSuccess: (data) => { setCreatedId(data); },
    onError: (e) => toast.error(e.message),
  });
  const updateMut = trpc.ads.update.useMutation({
    onSuccess: () => { utils.ads.listAll.invalidate(); toast.success("Ad updated"); onClose(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-display text-xl">{ad ? "Edit Ad Slot" : "New Ad Slot"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Input type="url" value={form.destinationUrl} onChange={(e) => setForm({ ...form, destinationUrl: e.target.value })} required placeholder="https://..." />
          </div>
          <div className="space-y-1.5">
            <Label>Position *</Label>
            <Select value={form.position} onValueChange={(v) => setForm({ ...form, position: v as AdFormData["position"] })}>
          ...
          <div className="space-y-1.5">
            <Label>Sort Order <span className="text-muted-foreground font-normal">(lower = first)</span></Label>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Manage business advertising placements on the directory.</p>
        </div>
        <Button onClick={() => { setEditAd(undefined); setDialogOpen(true); }}>
          <Plus className="h-4 w-4 mr-2" />
          New Ad Slot
        </Button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{POSITION_LABELS[ad.position]}</p>
                  </div>
                  <Switch
                    checked={ad.isActive}
                    onCheckedChange={(v) => toggleMut.mutate({ id: ad.id, isActive: v })}
                  />
                </div>
                <a href={ad.destinationUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1 mb-3 truncate">
                  <ExternalLink className="h-3 w-3 flex-shrink-0" />
                  {ad.destinationUrl}
                </a>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="flex-1" onClick={() => { setEditAd(ad); setDialogOpen(true); }}>
                    <Pencil className="h-3.5 w-3.5 mr-1.5" />
                    Edit
                  </Button>
                  <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => setDeleteId(ad.id)}>
          ...
          {ads?.length === 0 && (
            <div className="col-span-full text-center py-16 bg-card rounded-xl border border-border">
              ...
              <h3 className="font-display text-lg font-semibold">No ad slots yet</h3>
              <p className="text-muted-foreground mt-1">Create your first ad slot to start monetizing the directory.</p>
            </div>
          )}
        </div>
      )}

      <AdDialog open={dialogOpen} onClose={() => setDialogOpen(false)} ad={editAd} />

      <AlertDialog open={deleteId !== null} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this ad slot?</AlertDialogTitle>
            <AlertDialogDescription>This will permanently remove the ad from the directory.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center max-w-sm">
          ...
          <h1 className="font-display text-2xl font-bold mb-2">Owner Access Required</h1>
      ...
      <header className="bg-primary text-primary-foreground border-b border-primary/20">
        <div className="container py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-primary-foreground/60 uppercase tracking-widest">Admin Panel</p>
              <h1 className="font-display font-semibold leading-tight">Lambton Youth Sports Directory</h1>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" className="text-primary-foreground/70 hover:text-primary-foreground hover:bg-white/10" onClick={() => navigate("/")}>
              View Directory
              <ExternalLink className="h-3.5 w-3.5 ml-1.5" />
            </Button>
            <Button variant="ghost" size="sm" className="text-primary-foreground/70 hover:text-primary-foreground hover:bg-white/10" onClick={() => logout()}>
