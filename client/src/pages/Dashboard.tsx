  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {isListingsLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center">
            ...
            <p className="text-sm text-slate-400">
              Items you post on the Equipment Swap board will appear here.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {activeListings.map(listing => (
              <div
                key={listing.id}
                className="bg-white rounded-xl border border-slate-200 p-5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center"
              >
                <div>
                  <h3 className="font-bold text-lg">{listing.itemName}</h3>
                  <p className="text-sm text-slate-500">
                    {listing.sportCategory} • ${listing.price / 100}
                  </p>
        ...
        <h2 className="font-display text-xl font-bold mt-12 mb-6 text-slate-500">
          Past Listings ({completedListings.length})
        </h2>
        {completedListings.length > 0 && (
          <div className="space-y-3 opacity-60">
            {completedListings.map(listing => (
              <div
                key={listing.id}
                className="bg-slate-100 rounded-xl border border-slate-200 p-4 flex justify-between items-center"
              >
                <div>
