...
export default function SeasonCalendar() {
  ...
  const sportGroups = useMemo(() => {
    ...
    return Object.entries(groups)
      ...
      .map(([sport, progs]) => ({
        sport,
        programs: progs.sort((a: any, b: any) =>
  ...
  const getBarStyle = (startDate: string | null, endDate: string | null) => {
    ...
    const endDay = Math.floor(
      (clampedEnd.getTime() - yearStart.getTime()) / (1000 * 60 * 60 * 24)
    );
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              ...
              <p className="text-blue-200 text-lg font-medium max-w-xl">
                See every sport's registration windows and seasons at a glance.
                Plan your year without the guesswork.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 bg-white/10 rounded-xl px-4 py-2.5 border border-white/20 backdrop-blur-md">
              ...
              <span className="font-display font-bold text-2xl min-w-[80px] text-center">
                {year}
              </span>
              <button
                onClick={() => setYear(y => y + 1)}
            ...
            {sportGroups.map(({ sport, programs: progs }) => {
              ...
              return (
                ...
                >
                  {/* Sport Header */}
                  <div
                    className={`px-5 py-3 border-b border-slate-100 flex items-center gap-3 ${color.light}`}
                  >
                    <div className={`w-3 h-3 rounded-full ${color.bg}`}></div>
                    <h3
                      className={`font-display font-bold text-lg ${color.text}`}
                    >
                      {sport}
                    </h3>
                    <span className="text-xs font-bold text-slate-400 bg-white px-2 py-0.5 rounded-full border border-slate-200">
                      {progs.length}{" "}
                      {progs.length === 1 ? "program" : "programs"}
                    </span>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    {/* Program Rows */}
                    {progs.map((prog: any) => {
                      const regBar = getBarStyle(
                        prog.registrationOpenDate,
                        prog.registrationCloseDate
                      );
                      const seasonBar = getBarStyle(
                        prog.programStartDate,
                        prog.programEndDate
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="w-48 shrink-0 px-4 py-3 border-r border-slate-100 flex items-center">
                            <p
                              className="text-sm font-medium text-slate-700 truncate group-hover:text-blue-600 transition-colors"
                              title={prog.organization}
                            >
                              {prog.organization}
                            </p>
                          </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                      ...
                      <p className="font-semibold text-slate-800">
                        {formatDate(selectedProgram.registrationOpenDate)}
                      </p>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                      <p className="text-xs font-bold text-slate-400 uppercase mb-1">
                      ...
                      <p className="font-semibold text-slate-800">
                        {formatDate(selectedProgram.registrationCloseDate)}
                      </p>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                      <p className="text-xs font-bold text-slate-400 uppercase mb-1">
                      ...
                      <p className="font-semibold text-slate-800">
                        {formatDate(selectedProgram.programStartDate)}
                      </p>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                      <p className="text-xs font-bold text-slate-400 uppercase mb-1">
                      ...
                      <p className="font-semibold text-slate-800">
                        {formatDate(selectedProgram.programEndDate)}
                      </p>
                    </div>
                  </div>
